    ...
    <Link onClick={onClick} href={path} className={` ${className}`}>
      ...
      >
        <div
          ...
          hover:border-b-0 hover:h-11 focus:outline-none bg-button-${color}`}
        >
          <div className="flex items-center justify-center">{icon}</div>
        </div>

        <div
          className={`absolute items-center left-16 h-10 p-2 tooltip rounded-lg 
          pointer-events-none opacity-0 transform duration-300 group-hover:opacity-100 sm:flex hidden `}
        >
          <p
            className={`text-white text-shadow text-center w-full font-semibold font-jose`}
          >
            {capitalize(label)}
          </p>
        </div>
